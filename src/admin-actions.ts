...
export class AdminActions extends LitElement {
    ...
    async getAgents(){const myHeaders = new Headers();
myHeaders.append("Content-Type", "application/json");
myHeaders.append("Accept", "application/json");
myHeaders.append("Authorization", `Bearer ${this.token}`);

const raw = JSON.stringify({
  "query": "query activeAgents($from:Long! $to:Long! $filter:AgentSessionFilters $extFilter:AgentSessionSpecificFilters $pagination:Pagination){agentSession(from:$from to:$to filter:$filter extFilter:$extFilter pagination:$pagination){agentSessions{isActive agentId agentName agentSessionId agentSignOutReason userLoginId endTime startTime state siteId siteName teamId teamName orgId orgName multiMediaProfileType skillsProfile agentSkills parentOrgId parentOrgName channelInfo{currentState lastActivityTime}}pageInfo{endCursor hasNextPage}intervalInfo{interval timezone}}}",
  "variables": {
